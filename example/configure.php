...
if (file_exists($GLOBALS['AppConfig']['PhysicalPath'].'/configure.local.php')) {
    ...
    define('URL_FOLDERPATH', '/sumit/tas/tasphpapi/example');
    define('ADMIN_EMAIL', 'youremail@example.com');
}

/*
 * Disable Database for sample.
 */
...
$GLOBALS['AppConfig']['SiteName'] = 'API Demo';
...
set_include_path(get_include_path().PATH_SEPARATOR.$GLOBALS['AppConfig']['PhysicalPath']);
 ...
 //Remove for Database is not need in sample.
...
//REMARK: We include it here for this sample purpose. Actual usage will need you to add it in composer.json dependency.

require_once dirname(__FILE__).'/../src/APIRequest.php';
require_once dirname(__FILE__).'/../src/APIRequestHandler.php';
require_once dirname(__FILE__).'/../src/APIResponse.php';
